...
Процедура ТестДолжен_УказыватьРепозиторийВСсылкахНаIssueИPullRequestКоммита() Экспорт
    ...
    // Проверка
    Ожидаем.Что(Результат.Тело).Содержит("Fix oscript-library/autumn#13");

КонецПроцедуры

&Тест
Процедура ТестДолжен_НеПреобразовыватьНомерIssueСБуквеннымСуффиксом() Экспорт
    ...
    // Проверка
    Ожидаем.Что(Результат.Тело).Содержит("Fix #13abc");
